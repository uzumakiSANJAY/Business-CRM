...
import { Plus, Edit2, Trash2, Navigation, X } from 'lucide-react';
import Layout from '../../components/shared/Layout.jsx';
import ConfirmModal from '../../components/shared/ConfirmModal.jsx';
import { SkeletonRow } from '../../components/shared/LoadingSpinner.jsx';
import { useToast } from '../../components/shared/Toast.jsx';
import { getRoutes, createRoute, updateRoute, deleteRoute } from '../../api/routes.api.js';
...
function RouteModal({ route, onClose }) {
  ...
  const mutation = useMutation({
    mutationFn: (data) => isEdit ? updateRoute(route.id, data) : createRoute(data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['routes'] });
      toast(isEdit ? 'Route updated' : 'Route created', 'success');
      onClose();
    },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md animate-slide-in">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <h3 className="text-base font-semibold text-slate-900">{isEdit ? 'Edit Route' : 'Add Route'}</h3>
        ...
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Kandi Bazar, North Zone..."
...
export default function RoutesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['routes'] }); toast('Route removed', 'success'); setDeleteTarget(null); },
    onError: () => toast('Error removing route', 'error'),
  });

  return (
    <Layout title="Master Routes">
                  ...
                  <tr>
                    <td colSpan={4} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-slate-400 text-sm">No routes yet. Add one to organize vendors.</p>
                      </div>
                    </td>
                  </tr>
                )
                  ...
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="table-td">
                      <div className="flex items-center gap-2">
                        <Navigation className="h-3.5 w-3.5 text-indigo-400 flex-shrink-0" />
                        <span className="font-semibold text-slate-800">{r.name}</span>
                      </div>
                    </td>
                    <td className="table-td text-slate-500">{r.description || '—'}</td>
                    <td className="table-td text-slate-400 text-sm">{formatDate(r.created_at)}</td>
                    <td className="table-td">
      ...
      <ConfirmModal
        isOpen={!!deleteTarget}
        title="Remove Route"
        message={`Remove "${deleteTarget?.name}"? Vendors on this route will be unlinked.`}
